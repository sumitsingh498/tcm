import React from "react";
import { motion } from "framer-motion";
import { FileText, Palette, Ruler, HardHat, ArrowRight } from "lucide-react";
import { Link } from "react-router-dom";

export default function DesignBuild() {
  return (
    <section className="relative min-h-screen overflow-hidden bg-gradient-to-br from-gray-50 via-white to-gray-100">
      <div className="absolute inset-0">
        <div className="absolute bottom-20 left-20 w-80 h-80 bg-[#39A0ED]/10 rounded-full blur-3xl" />
      </div>

      <div className="relative z-10 px-6 pt-32 pb-20 text-center">
        <div className="mx-auto max-w-7xl">
          <motion.h1 initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }}>
            <h1 className="text-6xl font-black md:text-8xl lg:text-9xl">
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#39A0ED] to-[#2C74B3]">
                DESIGN & BUILD
              </span>
              <br />
              <span className="text-5xl text-gray-800 md:text-7xl">SERVICES</span>
            </h1>
          </motion.h1>
          <p className="max-w-5xl mx-auto mt-8 text-xl text-gray-600 md:text-2xl">
            One team. One contract. Zero stress — from concept sketches to final handover, we handle everything.
          </p>

          <div className="grid gap-8 mt-20 md:grid-cols-4">
            {[
              { icon: Palette, label: "Concept Design" },
              { icon: FileText, label: "Detailed Drawings" },
              { icon: Ruler, label: "BOQ & Estimation" },
              { icon: HardHat, label: "Execution & Handover" },
            ].map((step, i) => (
              <motion.div key={i} className="p-8 bg-white shadow-xl rounded-3xl">
                <step.icon className="w-14 h-14 mx-auto mb-4 text-[#39A0ED]" />
                <p className="text-lg font-bold">{step.label}</p>
                <div className="mt-2 text-5xl font-black text-gray-300">{i + 1}</div>
              </motion.div>
            ))}
          </div>

          <motion.div className="mt-24">
            <Link to="/contact" className="inline-flex items-center gap-4 px-14 py-7 bg-gradient-to-r from-[#39A0ED] to-[#2C74B3] text-white font-bold text-2xl rounded-full hover:scale-105 shadow-2xl">
              Start Design & Build <ArrowRight className="w-8 h-8" />
            </Link>
          </motion.div>
        </div>
      </div>
    </section>
  );
}